 ...
 */
struct callee
    : pegtl::sor<u, str_print, str_input, str_tuple_error, str_tensor_error> {};
struct oper : pegtl::seq<t, spaces, op, spaces, t> {};
struct var_index : pegtl::seq<var, spaces,
                              pegtl::plus<pegtl::one<'['>, spaces, index,
                                          spaces, pegtl::one<']'>>> {};
struct function_call : pegtl::seq<str_call, spaces, callee, spaces, pegtl::one<'('>,
                                  spaces, args, spaces, pegtl::one<')'>> {};
struct new_array : pegtl::seq<str_Array, spaces, pegtl::one<'('>, spaces, args, spaces,
                              pegtl::one<')'>> {};
struct new_tuple : pegtl::seq<str_Tuple, spaces, pegtl::one<'('>, spaces, t, spaces,
                              pegtl::one<')'>> {};
struct type : pegtl::sor<pegtl::seq<str_int64, pegtl::star<str_brackets>>,
                         str_tuple, str_code> {};
struct array_length : pegtl::seq<str_length, spaces, var, spaces, t> {};
struct tuple_length : pegtl::seq<str_length, spaces, var> {};
...
struct param : pegtl::seq<type, spaces, var> {};
 ...
 * Instructions
 */
struct inst_var_def : pegtl::seq<type, spaces, var> {};
struct inst_var_num_label_assign
    : pegtl::seq<var, spaces, op_arrow, spaces, s> {};
struct inst_op_assign : pegtl::seq<var, spaces, op_arrow, spaces, oper> {};
struct inst_index_load
    : pegtl::seq<var, spaces, op_arrow, spaces, var_index> {};
struct inst_index_store : pegtl::seq<var_index, spaces, op_arrow, spaces, s> {};
struct inst_array_length
    : pegtl::seq<var, spaces, op_arrow, spaces, array_length> {};
struct inst_tuple_length
    : pegtl::seq<var, spaces, op_arrow, spaces, tuple_length> {};
struct inst_call : pegtl::seq<function_call> {};
struct inst_call_assign
    : pegtl::seq<var, spaces, op_arrow, spaces, function_call> {};
struct inst_assign_new_array
    : pegtl::seq<var, spaces, op_arrow, spaces, str_new, spaces, new_array> {};
struct inst_assign_new_tuple
    : pegtl::seq<var, spaces, op_arrow, spaces, str_new, spaces, new_tuple> {};
...
struct bb_label : label {};
struct index : t {};

/*
 * High-level grammar rules
 */
struct inst : pegtl::sor<inst_var_def, inst_op_assign, inst_index_load,
                         inst_index_store, inst_array_length, inst_tuple_length,
                         inst_var_num_label_assign, inst_call, inst_call_assign,
                         inst_assign_new_array, inst_assign_new_tuple> {};

struct inst_list : pegtl::star<pegtl::seq<seps_with_comments, spaces, inst,
                                          spaces, seps_with_comments>> {};

struct te
    : pegtl::sor<inst_cond_branch, inst_branch, inst_return_val, inst_return> {
};

struct bb : pegtl::seq<spaces, bb_label, seps_with_comments, inst_list,
                       seps_with_comments, spaces, te> {};

struct bb_list : pegtl::plus<seps_with_comments, bb, seps_with_comments> {};

struct function
    : pegtl::seq<str_define, spaces, T, spaces, function_name, spaces,
                 pegtl::one<'('>, spaces, function_params, spaces,
                 pegtl::one<')'>, spaces, seps_with_comments, pegtl::one<'{'>,
                 bb_list, seps_with_comments, spaces, pegtl::one<'}'>> {};
...
struct action<callee> {
  ...
  static void apply(const Input& input, Program& program) {
    if (!enum_callee.empty()) {
      EnumCallee enumCallee = enum_callee.back();
      enum_callee.pop_back();
      parsed_items.push_back(std::make_unique<Callee>(enumCallee));
    } else {
      std::unique_ptr<ASTNode> item = std::move(parsed_items.back());
      parsed_items.pop_back();
      parsed_items.push_back(std::make_unique<Callee>(std::move(item)));
    }
  }
};
...
struct action<var_index> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    indexCounter = 0;
    std::unique_ptr<ASTNode> varItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    parsed_items.push_back(
        std::make_unique<Index>(std::move(varItem), std::move(indices)));
  }
};
...
template <>
struct action<array_length> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    std::unique_ptr<ASTNode> indexItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    std::unique_ptr<ASTNode> varItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    parsed_items.push_back(std::make_unique<ArrayLength>(std::move(varItem),
                                                         std::move(indexItem)));
  }
};

template <>
struct action<tuple_length> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    std::unique_ptr<ASTNode> varItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    parsed_items.push_back(std::make_unique<TupleLength>(std::move(varItem)));
  }
};
...
struct action<param> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    parsed_items.pop_back();
    parsed_items.push_back(
        std::make_unique<Parameter>(std::move(typeItem), std::move(varItem)));
  }
};

template <>
struct action<inst_var_def> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    std::unique_ptr<ASTNode> varItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    std::unique_ptr<ASTNode> typeItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<VarDefInstruction>(
        std::move(typeItem), std::move(varItem)));
  }
};

template <>
struct action<inst_var_num_label_assign> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    ...
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<VarNumLabelAssignInstruction>(
...
struct action<inst_op_assign> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<OperatorAssignInstruction>(
        std::move(dstItem), std::move(srcItem)));
  }
};
...
struct action<inst_index_load> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<IndexAssignInstruction>(
        std::move(dstItem), std::move(srcItem)));
  }
};
...
template <>
struct action<inst_array_length> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<LengthAssignInstruction>(
        std::move(dstItem), std::move(srcItem)));
  }
};

template <>
struct action<inst_tuple_length> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
...
struct action<inst_call_assign> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<CallAssignInstruction>(
        std::move(dstItem), std::move(functionCall)));
  }
};

template <>
struct action<inst_assign_new_array> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<NewArrayAssignInstruction>(
        std::move(dstItem), std::move(newArrayItem)));
  }
};

template <>
struct action<inst_assign_new_tuple> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    ...
    std::unique_ptr<ASTNode> dstItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    curBB->addInstruction(std::make_unique<NewTupleAssignInstruction>(
...
struct action<inst_branch> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    parsed_items.pop_back();
    curBB->addTE(std::make_unique<BranchInstruction>(std::move(targetItem)));
  }
};
...
struct action<inst_cond_branch> {
  ...
  static void apply(const Input& input, Program& program) {
    ...
    parsed_items.pop_back();
    curBB->addTE(std::make_unique<ConditionalBranchInstruction>(
...
struct action<inst_return> {
  ...
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    BasicBlock* curBB = curFunction->get_basicBlocks().back().get();
    curBB->addTE(std::make_unique<ReturnInstruction>());
  }
};
...
struct action<function_name> {
  ...
  static void apply(const Input& input, Program& program) {
    std::unique_ptr<ASTNode> typeItem = std::move(parsed_items.back());
    parsed_items.pop_back();
    std::unique_ptr<ASTNode> labelItem =
        std::make_unique<Label>(input.string());
    program.addFunction(
        std::make_unique<Function>(std::move(typeItem), std::move(labelItem)));
  }
};
...
template <>
struct action<bb_label> {
  template <typename Input>
  static void apply(const Input& input, Program& program) {
    Function* curFunction = program.getFunctions().back().get();
    curFunction->addBasicBlock(
        std::make_unique<BasicBlock>(std::make_unique<Label>(input.string())));
  }
};
